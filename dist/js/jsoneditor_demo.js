...
(function () {
  ...
  if (
    ...
    typeof window.mappingOverride === "object" &&
    !Array.isArray(window.mappingOverride)
  ) {
  ...
  var jeIframe =
    jeIframeEl.contentWindow ||
    jeIframeEl.contentDocument.document ||
    jeIframeEl.contentDocument;
  ...
  var isInvalidJson = function (code) {
    try {
      ...
      return (
        "Invalid Schema: " +
        e.message.charAt(12).toUpperCase() +
        e.message.slice(13)
      );
    }
    return false;
  ...
  var closeModal = function (e) {
    if (e.target === jeModal && jeModal.classList.contains("show-modal"))
      toggleModal();
  };
  ...
  var getIntersectMarkers = function (newRange, keyString) {
    return Object.filter(
      aceCodeEditor.getSession().getMarkers(true),
      function (marker) {
        return (
          marker.clazz == "readonly-highlight" &&
          marker.range.intersects(newRange) &&
          !(
            marker.range.isStart(newRange.end.row, newRange.end.column) &&
            keyString !== "delete"
          ) &&
          !(
            marker.range.isEnd(newRange.start.row, newRange.start.column) &&
            keyString !== "backspace"
          )
        );
      }
  ...
  var getInsideMarkers = function (row, column) {
    return Object.filter(
      ...
      function (marker) {
        return (
          marker.clazz == "readonly-highlight" &&
          marker.range.inside(row, column)
        );
      }
    );
  };
  ...
  var listExternalFilesUsed = function () {
    var src = [],
      tags = jeIframe.document.querySelectorAll(
        'head script:not([src=""]), head link[rel="stylesheet"]:not([href=""])'
      );
    for (var tag in tags) {
    ...
    var listToClipboardHandler = function (val) {
      jeModalContent.innerText = copyToClipboard(val)
    ...
    jeModalContent
      .querySelector("#cbreq-list")
      .addEventListener(
        "click",
        listToClipboardHandler.bind(null, src.sort().join("\n")),
        { once: true }
      );
    toggleModal();
  };
  ...
  var setPanelHandler = function (panel) {
    var panelClose = panel.querySelector(".panel-close-button"),
      ...
      panelHandler = function (e) {
        ...
        if (
          panel.classList.contains("panel-inactive") &&
          panel.classList.contains("panel-left")
        ) {
          // panel closing
  ...
  var tabsHandler = function (e) {
    if (e.target && e.target.nodeName == "BUTTON") {
      ...
      for (var i = 0; i < buttons.length; i++) {
        ...
        document
          .querySelector(buttons[i].dataset.content)
  ...
  window.iframeOutputCatcher = function (json, validation_errors, na) {
    ...
    if (na !== 1) {
      ...
      var val = validation_errors.length
        ? validation_errors
        : { schema: "valid" };
      jeValidationStatus.classList.remove("schema-invalid");
      if (validation_errors.length)
        jeValidationStatus.classList.add("schema-invalid");
  ...
  var getJsonEditorOptions = function () {
    var options = {},
      ...
      els = jeCfg.querySelectorAll(
        'input[type="checkbox"]' + exclude + ",select" + exclude
      );
  ...
  var updateDirectLink = function (e) {
    ...
    if (e.target == jeDirectLink) {
      url +=
        ...
        encodeURIComponent(
          window.LZString.compressToBase64(
            JSON.stringify(aceSchemaEditor.getValue().trim())
          )
        );
      ...
      url +=
        ...
        encodeURIComponent(
          window.LZString.compressToBase64(
            JSON.stringify(aceStartvalEditor.getValue().trim())
          )
        );
      url +=
        "&code=" +
        encodeURIComponent(
          window.LZString.compressToBase64(
            JSON.stringify(aceCodeEditor.getValue().trim())
          )
        );
      url +=
        "&style=" +
        encodeURIComponent(
          window.LZString.compressToBase64(
            JSON.stringify(aceStyleEditor.getValue().trim())
          )
        );
      ...
      if (window.location.protocol !== "file:") {
        ...
        if (encUrl.length < 5000) {
          loadJSONP(
            "https://is.gd/create.php?format=json&url=" + encUrl,
            function (data) {
              // Clipboard actions not allowed here since it's a callback event and not an "User generated event"
              jeModalContent.innerHTML =
                '<div class="cbreq"><h3>Direct Link Generated</h3><button id="cbreq-url">Copy Url to Clipboard</button> <button id="cbreq-shorturl">Copy ShortUrl to Clipboard</button></div>';
              jeModalContent
                .querySelector("#cbreq-url")
                .addEventListener(
                  "click",
                  urlToClipboardHandler.bind(null, url),
                  { once: true }
                );
              jeModalContent
                .querySelector("#cbreq-shorturl")
                .addEventListener(
                  "click",
                  urlToClipboardHandler.bind(null, data.shorturl),
                  { once: true }
                );
              toggleModal();
            }
  ...
  var updateFromFile = function (response) {
    var example = JSON.parse(response),
      ...
      startval =
        Object.keys(example.startval).length !== 0
          ? JSON.stringify(example.startval, null, 2)
          : "",
      cfg = example.config,
      code = example.code,
      style = example.style,
    ...
    if (desc !== "" && desc != "Add optional description here. (HTML format)") {
      jeModalContent.innerHTML = jeExampleDesc.innerHTML =
        '<h3>Info about "' + example.title + '" Example</h3>' + desc;
      toggleModal();
    }
    ...
    if (params.schema) {
      try {
        aceSchemaEditor.setValue(
          JSON.parse(
            window.LZString.decompressFromBase64(
              decodeURIComponent(params.schema)
            )
          )
        );
        aceSchemaEditor.session.getSelection().clearSelection();
  ...
  var getCode = function (schema, startval) {
    var opt =
      "schema:" + schema + (startval.trim() ? ", startval:" + startval : "");
  ...
  var handleDrop = function (e) {
    ...
    reader.onload = function (e) {
      ...
      if (err) {
        ...
        Object.getOwnPropertyNames(JSON.parse(response)).sort().join(",") !==
        "code,config,desc,schema,startval,style,title"
      ) {
  ...
  var buildExtFiles = function (options, code) {
    ...
    for (var i in options) {
      if (
        ...
        options[i] &&
        (mapping.ext_lib[i] || (mapping[i] && mapping[i][options[i]]))
      ) {
        map = mapping.ext_lib[i] || mapping[i][options[i]];
        if (map.js)
          jsFiles = jsFiles.concat(
            typeof map.js === "string" ? [map.js] : map.js
          );
        if (map.css)
          cssFiles = cssFiles.concat(
            typeof map.css === "string" ? [map.css] : map.css
          );
      }
    }
    ...
    cssFiles = cssFiles.concat(
      getUserIncludes(code, /\s*\/\/\s*includeCSS\((['"])([^"']*)\1\)/g)
    );
    jsFiles = jsFiles.concat(
      getUserIncludes(code, /\s*\/\/\s*includeJS\((['"])([^"']*)\1\)/g)
    );
    ...
    if (cssFiles.length)
      extFiles +=
        ...
        cssFiles.join('" /><link rel="stylesheet" href="') +
        '" />';
    if (jsFiles.length)
      extFiles +=
        '<script src="' +
        jsFiles.join('"></script><script src="') +
        '"></script>';
  ...
  var buildEditorOptions = function (options) {
    ...
    for (var i in options) {
      if (options.hasOwnProperty(i) && !/^lib_/.test(i)) {
        var val =
          typeof options[i] === "string" ? '"' + options[i] + '"' : options[i];
  ...
  var getExamplesIndex = function () {
    ...
    loadFile("examples/index.json", "application/json", function (cfg) {
      if (isInvalidJson(cfg)) {
        ...
        jeShowLoadExample.title =
          "No examples available due to invalid index file";
      } else {
  ...
  var iframeReady = function () {
    ...
    eventFire(
      document.querySelector("nav.tabs button:nth-of-type(1)"),
      "click"
    );
  };
  ...
  var generateForm = function (e) {
    ...
    var code =
      getCode(aceSchemaEditor.getValue(), startVal) + aceCodeEditor.getValue();
...
*/
  ...
  aceCodeEditor.keyBinding.addKeyboardHandler({
    handleKeyboard: function (data, hash, keyString, keyCode, event) {
      ...
      var newRange = aceCodeEditor.getSelectionRange(),
        block =
          Object.keys(getIntersectMarkers(newRange, keyString)).length > 0;
      if (keyString == "return" && block)
        aceCodeEditor.selection.moveTo(newRange.end.row + 1, 0);
  ...
  aceCodeEditor.on(
    "cut",
    function (e) {
      if (Object.keys(getIntersectMarkers(e)).length)
        e.setEnd(e.start.row, e.start.column);
    },
    false
  );
  ...
  aceCodeEditor.on(
    ...
    function (e) {
      if (
        Object.keys(getIntersectMarkers(aceCodeEditor.getSelectionRange()))
          .length
      )
        e.text = "";
    },
  ...
  aceCodeEditor.$tryReplace = (function (orgFunc) {
    return function (newRange) {
      return Object.keys(getIntersectMarkers(newRange)).length
        ? null
  ...
  session.moveText = (function (orgFunc) {
    return function (fromRange, toPosition) {
      ...
      var block =
        Object.keys(getInsideMarkers(toPosition.row, toPosition.column))
          .length > 0;
      // Does the content being moved contain any markers?
      if (!block)
        block = Object.keys(getIntersectMarkers(fromRange)).length > 0;
  ...
  jeUploadExample.addEventListener(
    "click",
    eventClickFire.bind(null, jeFileUpload),
    false
  );
